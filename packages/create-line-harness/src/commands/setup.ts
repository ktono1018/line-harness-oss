...
export async function runSetup(
  ...
  p.intro(pc.bgCyan(pc.black(" PostyLoop セットアップ ")));
...
VALUES (${q(id)}, ${q(state.lineChannelId!)}, ${q("PostyLoop")}, ${q(state.lineChannelAccessToken!)}, ${q(state.lineChannelSecret!)}, 1, ${q(jstNowStr)}, ${q(jstNowStr)})
...
`;
  ...
  p.outro(
    pc.green(
      release
        ? `PostyLoop v${release.release.version} を使い始めましょう 🎉（更新: npx create-line-harness update）`
        : "PostyLoop を使い始めましょう 🎉",
    ),
  );
}
